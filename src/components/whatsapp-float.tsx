...
export function WhatsAppFloat({ href, title, body, sansClassName }: Props) {
  ...
  return (
    <div
      className="fixed bottom-4 right-4 z-[60] flex max-w-md flex-row items-end justify-end gap-2 sm:bottom-6 sm:right-6"
      onMouseEnter={() => setOpen(true)}
      onMouseLeave={() => setOpen(false)}
    >
      {open && (
        <div
          className={`${sansClassName} order-1 mb-0.5 max-w-xs rounded-2xl border border-zinc-200/80 bg-white p-3.5 pr-2 text-left text-sm text-zinc-800 shadow-2xl sm:p-4`}
        >
          <p className="font-semibold text-zinc-900">{title}</p>
          <p className="mt-1 text-zinc-600 leading-relaxed">{body}</p>
        </div>
      )}
      <a
        href={href}
